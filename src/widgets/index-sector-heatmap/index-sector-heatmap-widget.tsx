/**
 * Index Sector Heatmap Widget
 * Treemap visualization of TASE index constituents grouped by sector -> sub-sector -> symbol.
 * Rectangles sized by marketCap, colored by change %. Click to drill down.
 * Fork of market-sector-heatmap with an index selector dropdown.
 */
...
interface IndexSectorHeatmapResponse {
  tradeDate: string;
  indexId: number;
...
function extractHeatmapData(result: CallToolResult | null | undefined): IndexSectorHeatmapResponse | null {
  try {
    ...
    if (parsed?.items && Array.isArray(parsed.items)) return parsed as IndexSectorHeatmapResponse;
    return null;
  } catch {
    return null;
  }
...
function IndexHeatmapApp() {
  const { t } = useLanguage();
  const [data, setData] = useState<IndexSectorHeatmapResponse | null>(null);
  ...
  const { app, error } = useApp({
    appInfo: { name: "Index Sector Heatmap", version: "1.0.0" },
    capabilities: {},
    onAppCreated: (app) => {
  ...
  useEffect(() => {
    ...
    app.callServerTool({ name: "get-index-sector-heatmap-data", arguments: toolInput })
      .then((result) => {
        if (handleSubscriptionRedirect(result, app, setSubscribeUrl)) return;
        const d = extractHeatmapData(result);
        if (d) setData(d);
  ...
  return <IndexHeatmapInner app={app} data={data} setData={setData} hostContext={hostContext} toolInput={toolInput} />;
}

// ── Inner Component ────────────────────────────────────────────────────────────

interface IndexHeatmapInnerProps {
  app: App;
  data: IndexSectorHeatmapResponse | null;
  setData: React.Dispatch<React.SetStateAction<IndexSectorHeatmapResponse | null>>;
...
function IndexHeatmapInner({ app, data, setData, hostContext, toolInput }: IndexHeatmapInnerProps) {
  ...
  const handleRefresh = useCallback(async (tradeDate?: string, period?: HeatmapPeriod, indexId?: string) => {
    ...
    try {
      ...
      args.indexId = Number(indexId ?? selectedIndexId);
      const result = await app.callServerTool({ name: "get-index-sector-heatmap-data", arguments: args });
  ...
  const handleIndexChange = useCallback((val: string) => {
    ...
    handleRefresh(selectedDate || undefined, selectedPeriod, val);
  }, [selectedDate, selectedPeriod, handleRefresh]);
