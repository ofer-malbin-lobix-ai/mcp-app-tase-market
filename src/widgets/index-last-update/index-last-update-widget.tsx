/**
 * Index Last Update Widget
 * Displays real-time last-update trading data for index constituents.
 * Clone of market-last-update with index selector dropdown.
 */
...
import styles from "../market-last-update/market-last-update-widget.module.css";
...
const TOOL_NAME = "get-index-last-update-data";
...
function IndexLastUpdateApp() {
  ...
  const { app, error } = useApp({
    appInfo: { name: "Index Last Update", version: "1.0.0" },
    capabilities: {},
    onAppCreated: (app) => {
...
interface IndexLastUpdateAppInnerProps {
  app: App;
...
function IndexLastUpdateAppInner({
  app,
  data,
  setData,
  hostContext,
}: IndexLastUpdateAppInnerProps) {
  const { language, dir, toggle, t } = useLanguage();
  const [isRefreshing, setIsRefreshing] = useState(true);
  const [refreshError, setRefreshError] = useState<string | null>(null);
  const [selectedIndexId, setSelectedIndexId] = useState("137");
  ...
  return (
    <WidgetLayout title={t("home.tool.indexLastUpdate")} app={app} hostContext={hostContext} language={language} dir={dir} onLanguageToggle={toggle}>

      <NavRow app={app} items={NAV_BUTTONS} />
      ...
      {data && rows.length === 0 ? (
        ...
        <DataTable
          ...
          columns={columns}
          initialPageSize={50}
          storageKey="tase-index-last-update-column-visibility"
