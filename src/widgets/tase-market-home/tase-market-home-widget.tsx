...
function GuidePanel({ t, app }: { t: (key: TranslationKey) => string; app: NonNullable<ReturnType<typeof useApp>["app"]> }) {
  return (
  ...
  return (
    <WidgetLayout title={t("home.title")} app={app} hostContext={hostContext} titleClassName={styles.title} language={language} dir={dir} onLanguageToggle={toggle}>
      ...
      <div className={styles.metaRow}>
        <button className={styles.referenceToggle} onClick={() => { setRefOpen(!refOpen); if (!refOpen) setGuideOpen(false); }}>
          <span className={`${styles.referenceArrow} ${refOpen ? styles.referenceArrowOpen : ""}`}>&#9654;</span>
          {t("home.reference")}
        </button>
        <button className={styles.guideToggle} onClick={() => { setGuideOpen(!guideOpen); if (!guideOpen) setRefOpen(false); }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M2 2.5C2 2.22 2.22 2 2.5 2H6.5C7.33 2 8 2.67 8 3.5V14L7.75 13.75C7.42 13.58 7.04 13.5 6.65 13.5H2.5C2.22 13.5 2 13.28 2 13V2.5Z" fill="currentColor" opacity="0.5"/>
            <path d="M14 2.5C14 2.22 13.78 2 13.5 2H9.5C8.67 2 8 2.67 8 3.5V14L8.25 13.75C8.58 13.58 8.96 13.5 9.35 13.5H13.5C13.78 13.5 14 13.28 14 13V2.5Z" fill="currentColor"/>
          </svg>
          {t("home.guide")}
        </button>
      </div>
      {refOpen && <div className={styles.referenceContent}><ReferencePanel t={t} /></div>}
      {guideOpen && <div className={styles.referenceContent}><GuidePanel t={t} app={app} /></div>}
