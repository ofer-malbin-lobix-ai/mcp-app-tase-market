...
function deriveTitle(toolName: string): string {
  return toolName
    .replace(/^get-/, "")
    .replace(/-end-of-days?-data$/, "")
    .replace(/-/g, " ")
    .replace(/\b\w/g, (c) => c.toUpperCase())
    + " Candlestick";
}

function derivePeriodToolName(toolName: string): string {
  return toolName.replace(/-end-of-days?-data$/, "-period-data");
}
...
export function renderSymbolsCandlestickApp(config: SymbolsCandlestickConfig) {
  createRoot(document.getElementById("root")!).render(
    <StrictMode>
      <SymbolsCandlestickApp config={config} />
    </StrictMode>
  );
}
