...
import { APP_ID, getAppSubscription } from './types.js';
...
export async function checkSubscription(userId: string): Promise<boolean> {
  ...
  try {
    ...
    // Check for manual (prepaid) subscription — set via Clerk dashboard
    if (metadata.manual_subscription && metadata.expires_at) {
      const expiresAt = new Date(metadata.expires_at);
      const now = new Date();
      if (expiresAt > now) {
        console.log(`[Subscription] User ${userId} has active manual subscription until ${metadata.expires_at}`);
        subscriptionCache.set(userId, { active: true, checkedAt: Date.now() });
        return true;
      } else {
        console.log(`[Subscription] Manual subscription expired for ${userId} (expired: ${metadata.expires_at})`);
    ...
    if (metadata.free_trial && metadata.expires_at) {
      const expiresAt = new Date(metadata.expires_at);
      const now = new Date();
      if (expiresAt > now) {
        console.log(`[Subscription] User ${userId} has active free trial until ${metadata.expires_at}`);
        subscriptionCache.set(userId, { active: true, checkedAt: Date.now() });
        return true;
      } else {
        console.log(`[Subscription] Free trial expired for ${userId} (expired: ${metadata.expires_at})`);
        subscriptionCache.set(userId, { active: false, checkedAt: Date.now() });
        return false;
      }
    }

    // Check if user has PayPal subscription metadata
